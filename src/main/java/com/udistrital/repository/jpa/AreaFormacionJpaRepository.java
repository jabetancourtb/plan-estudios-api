...
public interface AreaFormacionJpaRepository extends JpaRepository<AreaFormacion, Integer> {
	...
	@Query(value = "SELECT af.* FROM area_formacion af WHERE af.nombre LIKE CONCAT('%', :nombre, '%')", nativeQuery = true)
	...
	@Query(value = "SELECT af.* FROM area_formacion af WHERE af.id_campo_formacion = :idCampoFormacion", nativeQuery = true)
